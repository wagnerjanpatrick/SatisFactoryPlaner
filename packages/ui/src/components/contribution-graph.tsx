...
// WeekDay type: 0 = Sunday, 1 = Monday, ..., 6 = Saturday
...
const fillHoles = (activities: Activity[]): Activity[] => {
  ...
  const startDate = dayjs(firstActivity.date);
  const endDate = dayjs(lastActivity.date);
...
const groupByWeeks = (
  ...
  // Calculate how many days to pad at the start to align with weekStart
  let daysToPad = firstDayOfWeek - weekStart;
  if (daysToPad < 0) {
    daysToPad += 7;
  }

  const paddedActivities = [
    ...(new Array(daysToPad).fill(undefined) as Array<Activity | undefined>),
...
const getMonthLabels = (
  ...
  weeks
    .reduce<MonthLabel[]>((labels, week, weekIndex) => {
      ...
      const monthIndex = dayjs(firstActivity.date).month();
...
export const ContributionGraphCalendar = ({
  ...
  return (
    <div
      className={cn(
        "-mx-4 w-[calc(100%+2rem)] overflow-x-auto px-4 sm:mx-0 sm:w-full sm:overflow-visible sm:px-0",
        className
      )}
      {...props}
    >
      <svg
        className="block h-auto min-w-[600px] overflow-visible sm:w-full sm:min-w-0"
        height={height}
