...
import { getConveyorTier, CONVEYOR_TIERS } from "@/data";
...
export default function ConveyorEdge(props: EdgeProps<ConnectionEdge>) {
	const {
		id,
		sourceX,
		sourceY,
	...
	const tier = getConveyorTier(currentTierId);
	const label = tier ? `${tier.throughput}/min` : "";
					...
					>
						<button
							...
							style={{
								fontSize: 10,
								fontWeight: 600,
								color: COLORS.belt.conveyor,
								background: "#1a1a1acc",
								padding: "1px 6px",
								borderRadius: 3,
								border: `1px solid ${COLORS.belt.conveyor}44`,
								cursor: "pointer",
								whiteSpace: "nowrap",
							}}
						>
						...
						{open && (
							<div
								style={{
									...
									marginTop: 4,
									background: "#1a1a1af0",
									border: `1px solid ${COLORS.belt.conveyor}66`,
									borderRadius: 4,
									overflow: "hidden",
									zIndex: 50,
									minWidth: 100,
								...
								{CONVEYOR_TIERS.map((t) => (
									<button
										...
										style={{
											...
											color: t.id === currentTierId ? COLORS.belt.conveyor : "#ccc",
											background: t.id === currentTierId ? `${COLORS.belt.conveyor}22` : "transparent",
											border: "none",
											cursor: "pointer",
											textAlign: "left",
											whiteSpace: "nowrap",
										...
										onMouseEnter={(e) => {
											(e.target as HTMLButtonElement).style.background = `${COLORS.belt.conveyor}33`;
										}}
										onMouseLeave={(e) => {
											(e.target as HTMLButtonElement).style.background =
												t.id === currentTierId ? `${COLORS.belt.conveyor}22` : "transparent";
										}}
									>
										Mk.{t.id.replace("mk", "")} — {t.throughput}/min
									</button>
								))}
							</div>
						)}
